 ...
 *
 */
function doRectanglesOverlap(rect1, rect2, secondCall) {
  if (
  ...
  if (!secondCall) return doRectanglesOverlap(rect2, rect1, true);
  return false;
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (buffer[str[i]] === 1) return str[i];
  }
  return null;
}
...
function isCreditCardNumber(ccn) {
  ...
  const sum = arr
    ...
    .map((el, i) => {
      if (i % 2 === 0) {
        let newEl = +el * 2;
        if (newEl > 9) newEl = Math.trunc(newEl / 10) + (newEl % 10);
        return newEl;
      }
      return +el;
    })
    .reduce((acc, el) => acc + el);
  if (last === (10 - (sum % 10)) % 10) return true;
  return false;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (stack.length === 0) {
      stack.push(str[i]);
    } else if (pare[stack[stack.length - 1]] === str[i]) {
      stack.pop();
    } else {
      stack.push(str[i]);
    }
  ...
  // if (result !== str) return isBracketsBalanced(result);
  // return false;
}
